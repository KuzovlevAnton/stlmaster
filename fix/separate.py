...
class Separator:
    ...
    def separate(polygons, accuracy):
        ...
        graph = Separator.numerize_graph(graph, ids)
    ...
    @staticmethod
    def numerize_graph(graph, ids):
        numerized_graph = {}
        for polygon, neighbours in graph.items():
            numerized_polygon = (ids[polygon[0]], ids[polygon[1]], ids[polygon[2]])
            numerized_neighbours = []
            for neighbour in neighbours:
                numerized_neighbour = (ids[neighbour[0]], ids[neighbour[1]], ids[neighbour[2]])
                numerized_neighbours.append(numerized_neighbour)
            numerized_graph[numerized_polygon] = numerized_neighbours
        return numerized_graph
